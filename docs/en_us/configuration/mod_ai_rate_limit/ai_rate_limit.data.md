...
| RateLimitPolicies{v}.rules.tpm[].redis_key | String | Stable Redis counter key | N | Generated by the control plane based on `(policy_id, rule_index)`; if not specified, BFE falls back to the original `name` logic | Non-empty string |
...
| RateLimitPolicies{v}.rules.rpm[].redis_key | String | Stable Redis counter key | N | Generated by the control plane based on `(policy_id, rule_index)`; if not specified, BFE falls back to the original `name` logic | Non-empty string |
