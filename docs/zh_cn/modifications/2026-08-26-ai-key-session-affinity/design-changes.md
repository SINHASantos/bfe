...
| `session_affinity_ttl` | int | 600 | 绑定空闲超时时间（秒）；命中绑定后会刷新 TTL，持续请求则绑定保持 |
...
`session_affinity_ttl` 是**空闲超时时间**，不是固定生命周期。只要同一个 `client_key_id` 在超时时间内持续请求，BFE 就会在命中绑定后调用 `Expire` 刷新 TTL；只有会话空闲超过该时间，绑定才会自动释放。
