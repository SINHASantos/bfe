...
验证当 `AIConf.ModelTable` 未配置 `Tiers` 时，BFE 退化到默认 `Prices` 计费，保持向后兼容。
...
3. mock 后端 `cluster_no_table` 已启动，返回 200 与如下 body：
   ...
   ```
4. 临时 BFE 配置已生成并加载，`cluster_no_table` 配置 `ModelTable`：
   - 无 `Tiers`；
...
- `cluster_no_table.AIConf.ModelTable.Tiers`：空
- `cluster_no_table.AIConf.ModelTable.Models[0]`：
  - `Model`: `deepseek-chat`
  - `Mode`: `chat`
...
- Redis 中 `quota:plan_rmb` 的余额变为：
  ...
  - 剩余 = `10000000000 - 20000 = 9999980000`
